    ...
    UINT_RANGE *r, const unsigned char *data)
{
     ...
     */
    if (pkt != NULL && keep_schunk_data_on_packet(fs, pkt, r) == 1) {
    ...
    const unsigned char *data)
{
    ...
    /*
     * Can not append, because existing chunk `sc` follows the new
     * chunk `r`.
     */
    ...
    if (r_local.start < sc->sc_range.end) {
        data += sc->sc_range.end - r_local.start;
        r_local.start = sc->sc_range.end;
    }

    /*
     * calculate how much data to append,
     * nothing to do when nothing to append.
     */
    ...
    offset = SCHUNK_SIZE(sc);
    memcpy(&sc->sc_data_w[offset], data, UINT64_TO_SIZE_T(rsize));
    ...
    const unsigned char *data)
{
    ...
    /*
     * Data can not be prepended, new data we try to prepend actually
     * follow the existing chunk sc.
     */
    ...
    /*
     * Calculate how much data to prepend,
     * nothing to do when nothing to prepend.
     */
    rsize = (r_local.end - r_local.start);
    if (rsize == 0)
        return 0;
    ...
    rsize = (rsize < dsize) ? rsize : dsize;
    memmove(&sc->sc_data_w[rsize], sc->sc_data,
        UINT64_TO_SIZE_T(SCHUNK_SIZE(sc)));

    /*
     * If only a part of `r` is being prepended, then
     * it must be from the end of `r`.
     */
    assert(r_local.start - r_local.end >= rsize);
    offset = r_local.end - rsize - r_local.start;
    memcpy(sc->sc_data_w, &data[offset], UINT64_TO_SIZE_T(rsize));
    sc->sc_range.start -= rsize;
    assert(SCHUNK_SIZE(sc) <= DIRECT_STORAGE_SZ);
    ...
    struct stream_chunk_t *sc)
{
    ...
    if (copy_sc != NULL) {
        DEBUG_PRINT(stderr, "%s %p [ %llu, %llu ] -> ",
            OPENSSL_FUNC, sc, sc->sc_range.start, sc->sc_range.end);
        sc->sc_range.end = sr->sr_range.start;
        /*
    ...
    struct stream_range_t *super_sr, struct stream_range_t *sub_sr)
{
    ...
    DEBUG_PRINT(stderr, "%s super: %p [ %llu, %llu ], sub: %p [ %llu, %llu]\n",
        OPENSSL_FUNC, super_sr, super_sr->sr_range.start, super_sr->sr_range.end,
    ...
    struct stream_range_t *left_sr, struct stream_range_t *right_sr)
{
    ...
    DEBUG_PRINT(stderr, "%s %p [ %llu, %llu ] + %p [ %llu, %llu ] = %p ",
        OPENSSL_FUNC, left_sr, left_sr->sr_range.start, left_sr->sr_range.end,
    ...
    const unsigned char *data, int fin)
{
    ...
    if ((sr = find_range(fs, &key_sr)) == NULL) {
        ...
        DEBUG_PRINT(stderr, "%s chunk: %p [ %llu, %llu ] new range: %p\n",
            OPENSSL_FUNC, sc, sc->sc_range.start, sc->sc_range.end, sr);
        sc = NULL;
    ...
    retransmit:
        ...
        /*
         * try to place chunk to direct storage first. The return code
         * indicates function stored data to direct storage. We don't
         * know how much data got stored. So we simply retransmit the
         * `r` and `data`, so it gets inserted to updated stream range `sr`
         */
        if (append_to_dstorage(sr, r, data) == 1)
            goto retransmit;

        /*
         * similar to append, but the new frame better fits to
         * start of the range `sr`
         */
         ...
         */
        if (sc->sc_range.start < sr->sr_range.start && sc->sc_range.end > sr->sr_range.end) {
            /* new chunk includes the whole range */
            if (sandwich_chunk(fs, sr, sc) == 0) {
                sr = NULL;
                goto err;
            }
        } else if (sc->sc_range.end > sr->sr_range.end && sc->sc_range.start <= sr->sr_range.end) {
            append_chunk(fs, sr, sc);
        } else if (sc->sc_range.start < sr->sr_range.start && sc->sc_range.end >= sr->sr_range.start) {
            prepend_chunk(fs, sr, sc);
        } else {
        ...
        while (adjacent_sr != NULL) {
            ...
            DEBUG_PRINT(stderr, "%s %p [ %llu, %llu ] >< %p [ %llu, %llu ]\n",
                OPENSSL_FUNC, sr, sr->sr_range.start, sr->sr_range.end, adjacent_sr,
...
int ossl_sframe_set_move_offset(SFRAME_SET *fs, uint64_t new_offset)
{
    ...
    DEBUG_PRINT(stderr, "%s offset: %llu -> %llu range: %p [ %llu, %llu ] -> ",
        OPENSSL_FUNC, fs->offset, new_offset, sr, sr->sr_range.start,
        sr->sr_range.end);
